<?php
    if(isset($_GET['email'])){
        $email=$_GET['email'];
    }
    session_start();
    if(isset($email))
    {
        $_SESSION["email"]=$email;
    }
    elseif(isset($_SESSION["email"]))
    {
        $email=$_SESSION["email"];
    }
    else{
        header('Location:./rog.php');
    }
    
    //　メールの表示
    //DB 
    @$db=mysqli_connect("mysql201.phy.lolipop.lan","LAA1395326","Xuj29z9k","LAA1395326-syuupure");
    if(!$db){
        die("DB接続エラー");
    }
    // 履歴メール.予約メールの表示
    $sear="SELECT * FROM wp20220207100038_mail ";
    $where=" WHERE email = '$email' order by num desc";
    $sear=$sear.$where.";";
    $retret=mysqli_query($db,$sear);
    if(!$retret){
        die("SQLエラー");
    }
    while($aaa=mysqli_fetch_assoc($retret))
    {
        $recrec[]=$aaa;
    }
    // チェックが入っているmailを格納
    $sear="SELECT * FROM wp20220207100038_mail ";
    $where=" WHERE email = '$email' AND checked = 'check' ";
    $sear=$sear.$where.";";
    $update=mysqli_query($db,$sear);
    if(!$update){
        die("SQLエラー");
    }
    while($iii=mysqli_fetch_assoc($update))
    {
        $ans_update[]=$iii;
    }
    // 予約メール用プログラム
    $sear="SELECT * FROM wp20220207100038_reservation ";
    $where=" WHERE EMAIL = '$email' ";
    $sear=$sear.$where.";";
    $ww=mysqli_query($db,$sear);
    if(!$ww){
        die("SQLエラー");
    }
    while($ooo=mysqli_fetch_assoc($ww))
    {
        $reserve[]=$ooo;
    }
    // 予約メール削除
    if(isset($reserve)){
        for($i=0;$i<count($reserve);$i++){
            if(isset($_GET["dele$i"])){
                $delete_num=$_GET["dele$i"];
                $dele="DELETE FROM wp20220207100038_reservation WHERE NUM = '$delete_num' AND EMAIL = '$email' ";
                $res=$dele.";";
                $ret=mysqli_query($db,$res);
                if(!$ret){
                    die("SQLエラー");
                }
                $ww=mysqli_query($db,$sear);
                if(!$ww){
                    die("SQLエラー");
                }
                $reserve=[];
                while($ooo=mysqli_fetch_assoc($ww))
                {
                    $reserve[]=$ooo;
                }
            }
        }
    }
    // 予約メールエラーメッセージ
    $current_date = date("Y-m-d");
    $coming_soon="";
    $sear="SELECT * FROM wp20220207100038_reservation ";
    $where=" WHERE EMAIL = '$email' ";
    $sear=$sear.$where.";";
    $ww=mysqli_query($db,$sear);
    if(!$ww){
        die("SQLエラー");
    }
    while($ooo=mysqli_fetch_assoc($ww))
    {
        $re[]=$ooo;
    }
    if(isset($re)){
        foreach($re as $i)
        {
            if($current_date <= $i['date'])
            {
                $day=$i['date'];
            }
            if($current_date == $i['date'])
            {
                $coming_soon="１日以内に送る予定のメールがあります！";
            }
            elseif(date("Y-m-d" , strtotime('+1 day')) == $i['date']){
                $coming_soon="１日以内に送る予定のメールがあります！";
            }
        } 
    }
    // お気に入り機能のflg作り
    if(isset($_SESSION['flg'])){
        $flg=1;
        // 更新された時のセッション
        $_SESSION['flg']=0;
    }
    else{
        $flg=0;
        $_SESSION['flg']=0;
    }
    $key=0;
    if(isset($recrec)){
        for($e=0;$e<=count($recrec);$e++){
            $check[$e]="";
            if(isset($recrec[$e]) && $flg == 0)
            {
                if($recrec[$e]['checked'] == "check")
                {
                    $check[$e]="checked";
                }
            }
            // 画面更新時 お気に入り追加ボタン
            if(isset($_GET["name$e"])){
                $check[$e] = $_GET["name$e"];
                $_SESSION["name$e"]=$_GET["name$e"];
            }
            // 削除ボタンを押したとき
            elseif(isset($_GET["delete_btn"]))
            {
                // データベースにチェック入っている物をここで出力
                if(isset($ans_update[$key]['checked'])){
                    if($e == $ans_update[$key]['num'])
                    {
                        $che=count($recrec) - $e -1;
                        $flg=0;
                        $check[$che] = "checked"; 
                        $key=$key+1;
                    }
                }                    
            }
            else
            {
                unset($_SESSION["name$e"]);
            }
            if($flg == 1){
            if(isset($recrec[$e])){
                // そのまま
                if($recrec[$e]['checked'] == "check" && $check[$e] == "checked")
                {
                }
                // 登録
                elseif($recrec[$e]['checked'] == null && $check[$e] == "checked")
                {
                    $num=count($recrec)-$e -1;
                    //更新
                    $sql=" UPDATE wp20220207100038_mail SET checked = 'check' WHERE NUM =  '$num' AND  EMAIL = '$email' ;";
                    $res=mysqli_query($db,$sql);
                    if(!$res){
                        die("SQLエラー".$res);
                    }
                }
                // 解除
                elseif($recrec[$e]['checked'] == "check" && $check[$e] != "checked")
                {
                    $num=count($recrec)-$e-1;
                    $sql=" UPDATE wp20220207100038_mail SET checked = '' WHERE NUM = '$num' AND  EMAIL = '$email' ";
                    $res=mysqli_query($db,$sql);
                    if(!$res){
                        die("SQLエラー".$res);
                    }
                }
                // そのまま
                else
                {
                }
            }
    }
}
}
///////////////////////////////////////////////////////////////////////////////////



?>

<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <!-- link -->
    <link rel="stylesheet" href="../css/home.css">
    <link rel="stylesheet" href="../css/destyle.css">
    <script src="https://code.jquery.com/jquery-3.5.1.min.js"></script>
    <title>ホーム</title>
</head>
<body>
    
    <div id="whole">
        <header>
            <img src="../img/aikon_white.png">
            <h1>HOBメール</h1>
        </header>
        <div class="left">
            <div class="function">
                <a class="create" href="mail.php">メール作成</a>
                <div class="up">
                    <a href="Receive.php?email=<?=$email?>">受信トレイ</a>
                    <p class="box">送信済みトレイ</p>
                    <a href="star.php?email=<?=$email?>">お気に入り</a> 
                    <a href="spam.php?email=<?=$email?>">迷惑メール</a>
                    <a href="trash.php?email=<?=$email?>">ゴミ箱</a>
                </div>
                <p>アカウント</p>
                <p><?=$email?></p>
                <a href="rog.php" class="out">ログアウト</a>
            </div>        
        </div>
        <div class="center">
            <div class="category">
                <p>― History ―</p>
                <p></p>
                <ul>
                    <li class="a">Title</li>
                    <li class="b">Content</li>
                    <li class="c">To</li>
                    <li class="d">Date</li>
                </ul>
            </div>
            <?php if(isset($recrec)){ ?>
                <div class="checkbox">
                    <form action="home.php" method="GET">
                        <ul>
                       <?php for($a=0;$a<count($recrec);$a++){?>
                        
                            <div class="check clear_box">
                                <li><input type="checkbox" name="name<?=$a?>" value="checked" <?=$check[$a]?>></li>
                            </div>
                            <div  class="mail">
                                <a href="page.php?num=<?=$recrec[$a]['num']?>&email=<?=$email?> ">
                                <li>
                                    <div class="a"><?=$recrec[$a]['title']?></div>
                                    <div class="b"><?=$recrec[$a]['content']?></div> 
                                    <div class="c"><?=$recrec[$a]['to_mail']?></div>
                                    <div class="d"><?=$recrec[$a]['date']?></div>
                                </li>
                            </div></a> 
                                                
        
                            <?php } ?>
                    <div class="submitbtn">
                        <input type="submit" value="お気に入り更新">
                    </div>
                </form>
                       </ul>
                </div><?php } ?>
        </div>
        <div class="right">
            <div class="category">
                <p>― Reminder ―</p>
                <p></P>
                <ul>
                    <li class="a">Title</li>
                    <li class="b">To</li>
                    <li class="c">Date</li>
                </ul>
            </div>
            <?php if(isset($coming_soon)){?>
                <h1 class="alert"><?php echo $coming_soon ; } ?></h1>
                <?php if(isset($reserve)){ ?>
                    <div class="delete">
                        <form action="home.php" method="GET">
                        <ul>
                            <?php for($i=0;$i<count($reserve);$i++){
                            if($current_date <= $reserve[$i]['date']){ ?>
                            <div class="check2 clear_box">
                                <li><input type="checkbox" name="dele<?=$i?>" value="<?=$reserve[$i]['num']?> check"></li>
                            <div>
                            <div  class="reserve">
                                <a href="reserve.php?num=<?=$reserve[$i]['num']?> ">
                                    <li>
                                        <div class="a"><?=$reserve[$i]['title']?></div> 
                                        <div class="b"><?=$reserve[$i]['to_mail']?></div>
                                        <div class="c"><?=$reserve[$i]['date']?></div>
                                    </li>
                            </div></a> <?php }}} ?>
                            <input class="submit" type="submit" name="delete_btn" value="削除">
                        </ul>
                        </from>
                    </div>
    </div>
    <script src="../js/home.js"></script>
</body>
</html>